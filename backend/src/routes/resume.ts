...
resumeRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (typeof resumeData !== "string" || !resumeData.startsWith("data:")) {
      res.status(400).json({ error: "resumeData must be a base64 data URL." });
      return;
    }
    ...
    try {
      ...
      res.status(502).json({ error: "Couldn't parse that resume. Try again." });
      return;
    }

    res.json({ fields: sanitizeParsedFields(parsed) });
  }),
);
